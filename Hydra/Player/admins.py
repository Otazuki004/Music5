...
from Hydra.config import BOT_USERNAME, GROUP_SUPPORT, IMG_3, UPDATES_CHANNEL, IMG_5
...
from Hydra.inline import stream_markup
...
@Client.on_message(
    command(["stops", f"stops@{BOT_USERNAME}", "end", f"end@{BOT_USERNAME}", "vstop"])
    & other_filters
)
@authorized_users_only
async def stop(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            clear_queue(chat_id)
            await m.reply(f"**Userbot Successfully left In VC✅**")
...
@Client.on_message(
    command(["aaastop", f"aaastop@{BOT_USERNAME}", "vmute"]) & other_filters
)
@authorized_users_only
...
@Client.on_message(
    command(["aaaplay", f"aaaplay@{BOT_USERNAME}", "vunmute"]) & other_filters
)
@authorized_users_only
